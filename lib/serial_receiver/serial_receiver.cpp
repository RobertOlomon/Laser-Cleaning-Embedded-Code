...
//recieved string from serial, parse to allowed Gcode and Mcode
  ...
  while (token != NULL) {
      if (token[0] == 'G') {
          ...
          // For M commands, if they have no additional parameter you can process directly.
...
SerialReceiver::SerialReceiver()
    : state_(State::WAITING_FOR_HEADER),
      ...
      lastReceivedFreeMessage_() {}


/**
 * @brief Parses incoming serial data and processes messages based on their type.
 * 
 * This function reads data from the serial buffer and processes it according to the current state.
 * It handles three states: WAITING_FOR_HEADER, READING_HEADER, and READING_BODY.
 * 
 * - WAITING_FOR_HEADER: Waits for the header byte (0xA5) to start reading a new message.
 * - READING_HEADER: Reads the message type and length from the header.
 * - READING_BODY: Reads the message body and processes it based on the message type.
 * 
 ...
 * 
 * It modyfies the lastReceivedCommandMessage_ and lastReceivedFreeMessage_ based on the message type.
 * and updates the lastReceivedMsgId_ with the message type.
 * The function updates the state machine and processes messages of type COMMAND.
 */
...
void SerialReceiver::parse() {
  switch (state_)
  {
    ...
    case State::READING_HEADER:
      ...
      {
        currMsgId_ = static_cast<MessageType>(Serial.read()); // Read the message type and set to current message id
      ...
      if (Serial.available() >= currMsgLen_)
      {
        ...
        switch (currMsgId_)
        {
          ...
          case MessageType::FREE:
            lastReceivedFreeMessage_ = FreeMessage(currMsgData_); // Kinda useless but here for completeness
            break;
