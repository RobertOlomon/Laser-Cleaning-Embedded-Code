...
// This code gives an easy way to configure the various driver setting of a Trinamic TMC5160-based
// driver.
...
// const uint32_t UART_BAUDRATE = 500000;  // UART baudrate : up to 750kbps with default TMC5160
// clock const uint8_t SPI_CS = 5;               // CS pin in SPI mode const uint8_t SPI_DRV_ENN =
// 8;          // DRV_ENN pin in SPI mode
...
//             Serial.print('/');
//         }
//         if (tokenCount > 0) Serial.print(tokens[tokenCount - 1]);
...
//             if (readChar == tokens[i]) answer = readChar;

//         if (answer == 0) Serial.print("Invalid character. ");
//     }
...
//         Serial1.setTimeout(2);  // TMC5160 should answer back immediately when reading a
//         register.
...
//     Serial.println("Driver and motor tuning is finished. To use the determined parameters you
//     can"); Serial.println("copy and paste the following code snippet: "); Serial.println();
...
//     Serial.print(motorParams.pwmGradInitial);
//     Serial.println(";");
//     Serial.println("motor.begin(powerStageParams, motorParams,
...
// void loop()
// {
//     // TODO serial commands => velocity / position operation and feedback (driver state,
//     position,
//     // etc)
...
//         while (Serial.available()) Serial.read();
//         Serial.println(
//             "Warning: the driver is not in its reset state. You may want to power cycle it then "
//             "press any key.");
//         while (!Serial.available())
//             ;
//     }
...
//     Serial.println(
//         "Let's choose the MOSFET gate driver current. This should be the lowest setting giving "
//         "slopes < 100ns");
...
//     Serial.println("");
//     Serial.println(
//         "Let's tune the Break Before Make time. This should be as short as possible but still "
//         "avoid overlapping of low side and high side MOSFETs.");
//     Serial.println(
//         "If the switching slope is in the range 40-80ns, the smallest setting (0, 0) can be
//         used.");
//     Serial.println(
//         "You should check the bridge output for a small plateau at -1.2V or VM + 1.2V and tune
//         its " "length");
...
//     while (!Serial.available())
//         ;
//     if (isDigit(Serial.peek())) powerParams->drvStrength = constrain(Serial.parseInt(), 0, 3);
...
//     while (!Serial.available())
//         ;
//     if (isDigit(Serial.peek())) powerParams->bbmTime = constrain(Serial.parseInt(), 0, 24);
...
//     while (!Serial.available())
//         ;
//     if (isDigit(Serial.peek())) powerParams->bbmClks = constrain(Serial.parseInt(), 0, 15);
...
//     while (!Serial.available())
//         ;
//     if (isDigit(Serial.peek())) senseResistor = Serial.parseFloat();
...
//     while (!Serial.available())
//         ;
//     if (isDigit(Serial.peek())) motorCurrent = Serial.parseFloat();
...
//     motorParams->irun = constrain(
//         floor(motorCurrent * 31.0 / (maxRmsCurrent * (float)motorParams->globalScaler / 256.0)),
//         0,
//         31);
...
//     Serial.print(
//         "Motor current standstill reduction ? Set to 0 for freewheeling / passive braking "
//         "(Default: ");
...
//     while (!Serial.available())
//         ;
//     if (isDigit(Serial.peek())) motorCurrentReduction = constrain(Serial.parseFloat(), 0.0, 1.0);

//     motorParams->ihold = constrain(floor((float)motorParams->irun * motorCurrentReduction), 0,
//     31); Serial.print("I_HOLD set to "); Serial.println(motorParams->ihold);

//     if (motorParams->ihold == 0)
//     {
//         Serial.println(
//             "Freewheeling options : 'n' for normal operation, 'f' for freewheeling, 'l' for "
//             "passive braking using LS drivers");
...
//     while (Serial.available()) Serial.read();
//     Serial.println(
//         "/!\\ WARNING : the determined parameters are only valid for this motor, at this
//         operating " "voltage.");
//     Serial.println("Please set the desired operating voltage now.");
//     Serial.println(
//         "The tuning procedure will start a medium speed motion for at least 2 full turns.");
...
//     Serial.println();
//     Serial.println(
//         "Starting operation with the determined current and default stealthChop values.");
...
//     motor->begin(*powerParams, *motorParams, TMC5160::NORMAL_MOTOR_DIRECTION);
//     motor->writeRegister(
//         TMC5160_Reg::TPOWERDOWN,
//         32);  // For Phase #1, the motor needs to be at standstill, with the run current.
...
//         delay(100);
//     } while (
//         !(abs((int32_t)motor->readRegister(TMC5160_Reg::XACTUAL)) > 10 &&
//           pwmScale.pwm_scale_auto == 0));  // Wait at least 10 steps.
...
//         {
//             if (pwmScale.pwm_scale_sum == 255 || pwmScale.pwm_scale_sum >= 4 *
//             pwmAuto.pwm_ofs_auto)
//             {
//                 tuningMaxSpeed /= 2.0;
//                 motor->setMaxSpeed(tuningMaxSpeed);
...
//     Serial.println();
//     Serial.println(
//         "/!\\ WARNING : You must run this tuning procedure again if the operating voltage "
//         "changes.");
...
//     while (!Serial.available())
//         ;
//     if (isDigit(Serial.peek())) motorParams->pwmOfsInitial = constrain(Serial.parseInt(), 0,
//     255);
...
//     while (!Serial.available())
//         ;
//     if (isDigit(Serial.peek())) motorParams->pwmGradInitial = constrain(Serial.parseInt(), 0,
//     255);
...
//     Serial.println();
//     Serial.println(
//         "Do you want to troubleshoot the power stage ? This will disable then enable again while
...
//             Serial.println();
//             Serial.println(
//                 "/!\\ Make sure that there is no real short condition and use a current limited "
//                 "power supply !");
