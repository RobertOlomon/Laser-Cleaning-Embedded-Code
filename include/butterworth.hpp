 ...
 *
 * @note This implementation is originally designed for constexpr, enabling compile-time
 *       computation for filter configurations, however the only compiler I have access to
 *       is for c++11 and this is dependent on c++20 constexpr functions.
 *
 ...
 *          ensure the bandwidth is less than fs/4 to avoid excessive peaking. If all
 *          else fails, check the coefficients in matlab/python. 
 *
 * @author Aiden Prevey
 * @date 4/29/2025
 * @version 1.1
 ...
 *
 * @code
 * static  Butterworth<1, LOWPASS> filter(wc, Ts);
 ...
 */
 ...
 /**
  * used to calculate the magnitude of a complex number
 ...
 /**
  * used to calculate the square root of a complex number
 ...
 public:
      ...
      */
      Butterworth(double wc, double Ts, double wh = 0.0)
